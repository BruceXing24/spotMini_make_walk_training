...
class CPG:
    def __init__(self, step_length=0.05, ground_clearance=0.05, ground_penetration=0.01, Tstance=0.6, Tswing=0.4):
        ...
        self.leg_initalxyz = [0.019, 0.055, -0.173]
    ...
    def trot_generator(self, t, direction, turn_radius):
        ...
        Txyz_FL = self.foot_trajectory(t, [self.init_x, self.init_y, self.init_z], direction, 'FL',
                                       turn_radius)
        Txyz_FR = self.foot_trajectory(t + self.Tswing, [self.init_x, self.init_y, self.init_z], direction,
                                       'FR', turn_radius)
        Txyz_BL = self.foot_trajectory(t + self.Tswing, [self.init_x, self.init_y, self.init_z], direction,
                                       'BL', turn_radius)
        Txyz_BR = self.foot_trajectory(t, [self.init_x, self.init_y, self.init_z], direction, 'BR',
                                       turn_radius)
    ...
    def walk_generator(self, t, direction, turn_radius):
        ...
        Txyz_FL = self.foot_trajectory(t, [0.019,0.055,-0.173], direction, 'FL',
                                       turn_radius)
        Txyz_FR = self.foot_trajectory(t + self.Tswing, [0.019,0.055,-0.173], direction,
                                       'FR', turn_radius)
        Txyz_BL = self.foot_trajectory(t + 2 * self.Tswing, [0.019,0.055,-0.173], direction,
                                       'BL', turn_radius)
        Txyz_BR = self.foot_trajectory(t + 3 * self.Tswing, [0.019,0.055,-0.173], direction, 'BR',
                                       turn_radius)
        FL_1, FL_2, FL_3 = self.spot_leg.IK_L_2(Txyz_FL[0], Txyz_FL[1], Txyz_FL[2])
        FR_1, FR_2, FR_3 = self.spot_leg.IK_L_2(Txyz_FR[0], Txyz_FR[1], Txyz_FR[2])
        BL_1, BL_2, BL_3 = self.spot_leg.IK_L_2(Txyz_BL[0], Txyz_BL[1], Txyz_BL[2])
...
if __name__ == '__main__':
    ...
    while True:
        ...
        for i in range(100):
            ...
            Txyz_FL = CPG_controller.foot_trajectory(t, CPG_controller.leg_initalxyz, 'left', 'FL', 0.8)
